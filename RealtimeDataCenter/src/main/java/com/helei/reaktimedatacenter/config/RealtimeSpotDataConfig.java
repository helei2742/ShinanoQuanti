...
@Data
@Component
@ConfigurationProperties(prefix = "shinano.quantity.realtime.spot")
public class RealtimeSpotDataConfig {
    ...
    public void setListenKLine(List<String> listenKLine) {
        realtimeKLineList = listenKLine.stream().map(s -> {
            ...
            Set<KLineInterval> set = Arrays.stream(split[1].split(",")).map(KLineInterval.STATUS_MAP::get).collect(Collectors.toSet());
